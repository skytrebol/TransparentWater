import {
	...
	Vector4,
	WebGLRenderTarget
} from './js/three.module.js'; //XXX PATH
...
var TransparentWater = function ( geometry, options ) {
	...
	var baseColor = new Color( options.baseColor !== undefined ? options.baseColor : 0x455258 );
	var intensity = new Color( options.intensity !== undefined ? options.intensity : 1.0 );
	...
	var mirrorShader = {
		...
		uniforms: UniformsUtils.merge( [
			...
			UniformsLib[ 'lights' ],
			{
				...
				'baseColor':  { value: new Color( 0x455258 ) },
				'intensity': { value: 1.0 }
			}
		] ),
...
//			'	vec3 reflectionSample = vec3( texture2D( mirrorSampler, mirrorCoord.xy / mirrorCoord.w + distortion ) );',
			...
			'       gl_FragColor = vec4( (gl_FragColor.r + baseColor.r)/2.0 * intensity, (gl_FragColor.g + baseColor.g)/2.0 * intensity, (gl_FragColor.b + baseColor.b)/2.0 * intensity, gl_FragColor.a );', // blend baseColor
	...
	scope.onBeforeRender = function ( renderer, scene, camera ) {
		...
		mirrorCamera.position.copy( new THREE.Vector3( 0, -1, 0 ) );
